...
State management is another area where fads have been taken to extremes. Atomic state is, of course, one of the great advances in frontend development in recent years, with Redux leading the way in simplifying what is arguably the great conceptual problem of frontend design. Therefore, without a doubt we can say: 

> Good idea: using atomic state to organize your app

  But what is sometimes forgotten is that, although atomic state may be a worthy endgoal, it is not necessarily the best intermediate stage. There are trade offs here involving a lot of boilerplate, as well as a commitment to developing your app using a particular model of the application state. That state may change later, as will the model, and it may be painful to change the state tree at that later date. Therefore: 

> Bad Idea: hardening state prior to having a sufficient idea of the requirements

What exactly is a "sufficient level" of requirements may of course be hard to precisely state. 

<img src="{{ site.url }}/images/monsoon.jpg" class="center" />

Note that if the goal is therefore to keep the Redux state tree as modular as possible, to ensure that development is agile, one might begin by giving each slice of the tree its own reducer. But this affordance to modularity already is a compromise---instead of modeling the final state as it should be, we are representing an intermediate model of the application state. Each reduced slice now begins to look like a separate service, but with more boilerplate spread across more files. Furthermore, if we now we need to share state across different slices of the state tree this will lead to further compromises, since they have been split apart for modularity. 

All of which is to simply say that the ability to reason about the state of the app granted by Redux is for naught if you don't actually know what the state of the app should be. So, as much as possible, the requirements should be defined up front, so that that application state model can be defined prior to beginning development. In this sense Redux is by necessity a solution involving front-loaded planning, and is more waterfall-esque than a free-flowing services model. 

So I think that if the desired UX is not defined to a certain degree of specificity, there should be no problem using a traditional services architecture to structure the frontend. The ultimate goal, atomic state, should be kept in mind for any app of significant complexity, however, because of the gains in reasoning about behavior. Instead of a binary atomic state / services model, it might be better to think in terms of a continuum, with services at one end and single-reducer Redux at the other end, our goal being to move as much as possible toward the side of atomic state, incrementally if necessary.  
